...
class Quiz extends Component {
  ...
  showQuestionOrAnswer = () => {
    const show = this.state.show === 'showanswer' ? 'answer' : 'showanswer';
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.quizCard}>
          ...
          {this.state.show == 'showanswer' ? (
            <Text style={styles.questionText}>{showingCard.answer}</Text>
          ) : (
            <Text style={styles.answerText}>See Possible Answer</Text>
          )}
          <ShowQuestionOrAnswer
...
function mapStateToProps(state, { navigation }) {
  ...
  return state[deckId] !== undefined ? state[deckId] : {};
}
